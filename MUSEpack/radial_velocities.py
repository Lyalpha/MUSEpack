...
logging.basicConfig(
    format="%(asctime)s [%(levelname)8s]:" + " %(message)s [%(funcName)s]",
    datefmt="%Y-%m-%d %H:%M:%S",
)
...
class RV_spectrum:
    ...
    def __init__(
        ...
        self.logger.info("Systematic RV shift: " + str("{:.2f}".format(self.rv_sys)) + " km/s")
    ...
    def plot(self, oversampled=False):
        ...
        nrows = int(len(self.cat.index) / 3.0) + 1
        ...
        exponent = int(np.log10(np.nanmedian(self.spec_f[non_inf_cont])) - 1.0)
        factor = float(10 ** (exponent * (-1)))
        ax_spec.plot(
            self.spec_lambda[non_inf_cont],
            self.spec_f[non_inf_cont] * factor,
            c="black",
            label="data",
            linewidth=2,
        )
        ...
        if not oversampled:
            ax_spec.plot(
                self.spec_lambda[non_inf_cont],
                self.template_f[non_inf_cont] * factor,
                c="red",
                label="template",
                linewidth=2,
            )
            ...
            ax_spec.plot(
                self.spec_lambda[non_inf_cont],
                self.fit_residuals[non_inf_cont] * factor,
                c="green",
                label="residual",
                linewidth=2,
            )
        ...
        if oversampled:
            ax_spec.plot(
                self.spec_lambda_highres[non_inf_cont_highres],
                self.template_f_highres[non_inf_cont_highres] * factor,
                c="red",
                label="template",
                linewidth=2,
            )
        ...
        ax_spec.set_ylabel(
            r"flux [$10^{" + str(exponent) + r"}$ erg/s/cm$^2$/${\rm \AA}$]", fontsize=12
        )
        ...
        for n in np.arange(len(self.cat.index)):
            ...
            ax_norm.plot(
                self.spec_lambda, self.spec_f / self.continuum, c="black", label="data", linewidth=2
            )

            if not oversampled:
                ax_norm.plot(
                    self.spec_lambda,
                    self.template_f / self.continuum,
                    c="red",
                    label="template",
                    linewidth=1.5,
                )
            ...
            ax_norm.annotate(
                el, (0.1, 0.5), xycoords="axes fraction", fontsize=12, fontweight="bold"
            )
    ...
    def line_fitting(
        ...
        max_contorder=2,
        max_ladjust=4,
        adjust_preference="contorder",
        input_continuum_deviation=0.05,
        llimits=[-2.0, 2.0],
        max_exclusion_level=0.3,
        blends=None,
        autoadjust=False,
        fwhm_block=False,
    ):
        ...
        warnings.filterwarnings(
            "ignore", category=RuntimeWarning, message="divide by zero encountered in true_divide"
        )
        warnings.filterwarnings(
            "ignore", category=RuntimeWarning, message="invalid value encountered in true_divide"
        )
        warnings.filterwarnings(
            "ignore", category=RuntimeWarning, message="invalid value encountered in greater_equal"
        )

        if isinstance(input_continuum_deviation, float):
            input_continuum_deviation = {
                line_idxs[i]: input_continuum_deviation for i in range(len(line_idxs))
            }
        assert len(input_continuum_deviation) == len(
            line_idxs
    ...
    def rv_fit_peak(self, line_sigma=3, line_significants=5):
        ...
        v = np.array(
            (self.cat.loc[:, "l_fit"].values / self.cat.loc[:, "l_lab"] - 1.0)
            * const.c.to("km/s").value
        )

        for i, vi in enumerate(v):
            self.logger.info(
                "Line " + self.cat.index[i] + ": RV=" + str("{:4.2f}".format(vi)) + " km/s"
            )
        ...
        if remaining_lines.mask.all():
            ...
            self.logger.info(
                "Finished Calculating RVs based on peaks only: "
                + "RV=("
                + str("{:4.2f}".format(self.rv_peak))
                + "+-"
                + str("{:4.3f}".format(self.erv_peak))
                + ")km/s based on "
    ...
    def rv_fit(
        self, guesses, niter=10000, line_sigma=3, n_CPU=-1, line_significants=5, RV_guess_var=0.0
    ):
        ...
        self.logger.info(
            "Settings: niter=" + str(niter) + "; sigma RV for excluding lines: " + str(line_sigma)
        )
        ...
        log_spec_f, logspec_lambda, velscale_spec = ppxf_util.log_rebin(
            [self.spec_lambda[0], self.spec_lambda[-1]], self.spec_f
        )

        log_template_f, log_template_lambda, velscale_template = ppxf_util.log_rebin(
            [self.spec_lambda[0], self.spec_lambda[-1]], self.template_f
        )

        log_spec_err, log_spec_err_lambda, velscale_spec_err = ppxf_util.log_rebin(
            [self.spec_lambda[0], self.spec_lambda[-1]], self.spec_err
        )

        log_spec_err[~np.isfinite(log_spec_err)] = 1.0
        # This happens if AO was used and there is a gap in the spec

        exponent = int(np.log10(np.nanmedian(log_spec_f)) - 4.0)
        ...
        fwhm_g, fwhm_l, fwhm_v = voigt_FWHM(
            self.cat.loc[:, "sg_fit"].values.astype(np.float64),
        ...
        for i, line in enumerate(l_lab):
            ...
            if self.cat.loc[line_name[i], "significance"] > line_significants:
                ...
                ind_min = (
                    np.argmin(np.abs(log_template_lambda - np.log(line - 0.5 * fwhm_v[i]))) - 1
                )
                ...
                log_spec_f[~np.isfinite(log_spec_f)] = 0.0

                pp_outliers_init = ppxf.ppxf(
                    log_template_f,
                    log_spec_f,
                    log_spec_err,
                    velscale_spec,
                ...
                self.logger.info(
                    "RV guess variation line "
                    + line_name[i]
                    + ": "
                    + str("{:4.2f}".format(RV_guess_var))
                    + "km/s"
                )
                ...
                self.logger.info(
                    ...
                    + str("{:4.2f}".format(v[i]))
                    + "+-"
                    + str("{:4.3f}".format(ev[i]))
                    + ")km/s"
                )

            else:
                self.logger.info(
                    line_name[i] + ": Line not significant [level =" + str(line_significants) + "]"
                )
                ev[i] = np.nan
                v[i] = np.nan
        ...
        if remaining_lines.mask.all():
            ...
            rv_var_lines = MAD(self.cat.loc[line_name[~remaining_lines.mask], "RV"])
            RV_guess_var_min = RV_guess_var
            if rv_var_lines > RV_guess_var:
                RV_guess_var = rv_var_lines

            self.logger.info(
                "RV guess variation: min = "
                + str(RV_guess_var_min)
                + "km/s; used = "
                + str("{:4.2f}".format(RV_guess_var))
                + "km/s"
            )
            ...
            self.rv, self.erv = ppxf_MC(
                ...
                guesses,
                nrand=niter,
                goodpixels=pp_final_init.goodpixels,
                degree=-1,
                moments=2,
            ...
            elapsed_time = time.time() - start_time
            self.logger.info(
                "Used lines for RV fit: " + ", ".join(line_name[~remaining_lines.mask])
            )

            self.logger.info(
                "Finished RV fitting: RV=("
                + str("{:4.2f}".format(self.rv))
                + "+-"
                + str("{:4.3f}".format(self.erv))
                + ")km/s based on "
                + str(len(l_fit))
                + "/"
                + str(len(line_name))

...
def line_fitter(
    ...
    resid_level = 5.0
    std_resid = resid_level + 1.0
    continuum_dev = input_continuum_deviation[line_idx] + 1.0
    ...
    while (
        std_resid > resid_level
        or np.isnan(std_resid)
        ...
        spec_select_idx = np.where((self.spec_lambda >= lstart) & (self.spec_lambda < lend))

        spec_select_idx_highres = np.where(
            (self.spec_lambda_highres >= lstart) & (self.spec_lambda_highres < lend)
        )

        linefit_guess, linefit_limits, linefit_limited = initial_guesses(
            self, lines_select, blends, llimits=llimits
        )

        exponent = int(np.log10(np.nanmedian(self.spec_f[spec_select_idx])) - 4.0)
        ...
        temp_lambda_highres = np.array(
            self.spec_lambda_highres[spec_select_idx_highres], dtype=float
        )
        ...
        while iterations < niter:
            if iterations == 1:
                newparinfo = update_parinfo(
                    self, linefit_guess, llimits, line_idx, blends, sp.specfit.parinfo, False, False
                )
            ...
            with log.log_to_list() as log_list:
                sp.baseline(
                    ...
                    exclusionlevel=float(exclusion_level),
                )
                ...
                if iterations == 0:
                    sp.specfit.multifit(
                        ...
                        verbose=False,
                        color=None,
                        guesses=list(linefit_guess),
                        parinfo=None,
                        reset_fitspec=True,
                        plot=False,
                        limits=linefit_limits,
                ...
                if (
                    len(log_list) > 0
                    and log_list[0].message[:8] == "gnorm=0."
                    and exclusion_level < max_exclusion_level
                ):
                    exclusion_level += 0.01
                    self.logger.info(
                        line_idx
                        + ": Adjusting exclusion level"
                        + " to: "
                        + str("{:2.2f}".format(exclusion_level))
                    )
                    iterations = 0
                    sp = pyspeckit.Spectrum(
                        data=temp_f,
                ...
                if len(log_list) == 0 or log_list[0].message[:8] != "gnorm=0.":
                    if iterations > 0:
                        ...
                        ) / sp.specfit.optimal_chi2(reduced=True, threshold="auto")

                        if (
                            sp.specfit.optimal_chi2(reduced=True, threshold="auto") == chi2
        ...
        if not fit_failed:
            ...
            for lab_line_idx, lab_lines in enumerate(np.array([self.cat.loc[line_idx, "l_lab"]])):
                par_extract_idx = np.concatenate(
                    (par_extract_idx, np.where(lines_select == lab_lines)[0])
                )
            ...
            for i in range(len(lines_select)):
                ...
                lowres_f = spec_res_downgrade(self.spec_lambda_highres, highres_f, self.spec_lambda)
                ...
                highres_f = voigt_funct(
                    self.spec_lambda_highres,
                    sp.specfit.parinfo[int(4 * i + 1)]["value"],
                    1,
                    sigma,
                    gamma,
                )
            ...
            continuum_dev = continuum_deviation(self, temp_lambda, temp_f, continuum, contorder)
            ...
            if (
                ...
                if max_cont_reached and max_n_ladjust_reached:
                    self.logger.warning(
                        line_idx + ": Maximum adjustments reached: Check the output"
                    )
                    break
                ...
                if adjust_preference == "contorder":
                    if not max_cont_reached:
                        contorder += 1
                        self.logger.info(
                            line_idx + ": Adjusting continuum order to: " + str(contorder)
                        )
                ...
                if adjust_preference != "contorder":
                    ...
                    if not max_n_ladjust_reached:
                        if (
                            adjust_preference == "min_lambda"
                            or adjust_preference == "minmax_lambda"
                        ):
                            lstart -= 5.0
                            self.logger.info(
                                line_idx + ": Adjusting lower wavelength range to: " + str(lstart)
                            )

                        if (
                            adjust_preference == "max_lambda"
                            or adjust_preference == "minmax_lambda"
                        ):
                            lend += 5.0
                            self.logger.info(
    ...
    if not fit_failed:
        ...
        if (
            temp_l - self.cat.loc[line_idx, "l_lab"] < 0.8 * llimits[0]
            ...
            self.logger.warning(
                ...
                + str(temp_l)
                + " llab: "
                + str(self.cat.loc[line_idx, "l_lab"])
                + " llimits = ["
                + str(llimits[0])
                + " "
                + str(llimits[1])
